...
TEST(CListTest, pop) {
	...
	CListTest_MockNode * n = nullptr;
	const i8 * s[] = {"g", "f", "e", "d", "c", "b", "a"};
	i32 i = 0;
	while (!lst.Empty()) {
		n = lst.PopBack();
		EXPECT_STREQ(n->value_.c_str(), s[i++]);
		delete n;
	}
}