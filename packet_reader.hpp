...
public:
    ...
    // 获取指定偏移量的指针
    inline uint8_t * GetPositionPtr(size_t pos) {
        return mem_ptr_ + pos;
    }
    ...
    inline PacketReader & operator >> (T & value) {
        if (sizeof(value) <= sizeof(int32_t)) {
            ...
            ReadBinary(reinterpret_cast<uint8_t *>(&value), sizeof(value));
        }
        return *this;
    }
...
protected:
    ...
    size_t RawReadStringLength(char * str, size_t len) {
        ...
        if (avaliable >= sizeof(LENGTH)) {
            ...
            if (read_len > 0) {
                ReadBinary(reinterpret_cast<uint8_t *>(str), read_len);
            }
