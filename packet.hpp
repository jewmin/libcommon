...
public:
    ...
    Packet(uint8_t * buf, size_t size) : PacketReader(buf, size), alloc_size_(0), size_(0) { data_end_ = mem_ptr_; }
    virtual ~Packet() { Free(); }

    // 设置内存增长大小
    inline void SetAllocSize(size_t size) { alloc_size_ = size; }
    // 获取当前缓冲长度下的可用字节数
    inline size_t GetAvaliableBufferLength() { return end_ptr_ - offset_; }

    // 设置数据包当前读写位置字节偏移量，如果新的偏移量比当前内存块长度要大，则函数会与SetLength一样进行内存块的扩展操作
    virtual size_t SetPosition(size_t new_pos) override
    {
        if (new_pos > GetSize()) Realloc(new_pos);
        offset_ = mem_ptr_ + new_pos;

        // 如果读写指针超过长度指针的位置，则调整长度指针为读写指针的位置
        if (offset_ > data_end_) data_end_ = offset_;
        return new_pos;
    }
    ...
    virtual size_t AdjustOffset(int64_t adjust_offset) override
    {
        offset_ += adjust_offset;
        if (offset_ < mem_ptr_) offset_ = mem_ptr_;
        else if (offset_ > end_ptr_) Realloc(offset_ - mem_ptr_);
    ...
    {
        if (new_length > GetSize()) Realloc(new_length);
    ...
    inline size_t Reserve(size_t new_size)
    {
        ...
        if (mem_size < new_size) Realloc(new_size);
        return mem_size;
    }
    ...
    inline void WriteAtom(const T & data)
    {
        ...
        if (mem_size < sizeof(data)) Realloc(GetSize() + sizeof(data));
    ...
    {
        size_t mem_size = GetAvaliableBufferLength();
        if (mem_size < size) Realloc(GetSize() + size);
...
protected:
    ...
    void Realloc(size_t new_size)
    {
        ...
        if (length > 0) memcpy(new_mem, mem_ptr_, __min(new_size, length));
        ...
        // 销毁原有数据内存
        if (old_mem && size_ > 0) jc_free(old_mem);
    ...
    void RawWriteStringLength(const char * str, size_t len)
    {
        ...
        if (len == (size_t)-1) len = str ? (LENGTH)strlen(str) : 0;
        else len = __min(len, strlen(str));
